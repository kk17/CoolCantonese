...
def update_config_with_json_config_file(config, config_filepath=None,
    ...
    if global_config_filepath:
        ...
        if path.exists(global_config_filepath):
            ...
            global_config = anyconfig.load(global_config_filepath)
            logger.debug("global config: %s", global_config)
    ...
    if not config_filepath:
        ...
        json_config = anyconfig.load(config_filepath)
...
class Config(AttrDict):
    ...
    def __init__(
            self, default_config_dict=None, config_filepath=None,
            global_config_filepath=None, enable_env_config=False):
        ...
        update_config_with_json_config_file(
            self, config_filepath, global_config_filepath)
